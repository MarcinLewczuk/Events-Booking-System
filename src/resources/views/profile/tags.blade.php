                                ...
                                <div class="mb-4">
                                    <div class="flex gap-2">
                                        <input
                                            ...
                                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#370671] focus:border-transparent"
                                        >
                                        <button
                                            type="button"
                                            onclick="filterTags()"
                                            class="px-4 py-2 bg-[#370671] text-white rounded-lg hover:bg-[#2b0553] transition"
                                        >
                                            Search
                                        </button>
                                    </div>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                    @foreach($allTags as $tag)
                                        <label
                                            ...
                                            class="relative flex items-center p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-purple-50 transition-colors
                                            {{ in_array($tag->id, $userTagIds) ? 'bg-purple-50 border-[#370671] ring-2 ring-[#370671]' : '' }}">
                                            <input type="checkbox" 
                                                   name="tags[]" 
                                                   value="{{ $tag->id }}"
                                            ...
                                            @if(in_array($tag->id, $userTagIds))
                                                <svg class="w-4 h-4 ml-auto text-[#370671]" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                                </svg>
                                            @endif
                                        </label>
            ...
            @if($userTagIds)
                <div class="mt-6 bg-purple-50 border border-purple-200 rounded-lg p-6">
                    ...
                    <p class="text-sm text-gray-600">
                        You're following <span class="font-bold text-[#370671]">{{ count($userTagIds) }}</span> 
