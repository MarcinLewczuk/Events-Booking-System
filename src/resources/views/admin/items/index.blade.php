        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-6 border-l-4 border-primary-900">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h1 class="text-3xl font-bold text-primary-900">Items Management</h1>
                        <p class="text-gray-600 mt-1">Manage your auction items and inventory</p>
                    </div>
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        @endif
                        <x-buttons.link :href="route('admin.items.create')">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                ...
                                <option value="intake" {{ request('status') == 'intake' ? 'selected' : '' }}>Intake</option>
                                <option value="photos" {{ request('status') == 'photos' ? 'selected' : '' }}>Photos</option>
                                <option value="description" {{ request('status') == 'description' ? 'selected' : '' }}>Description</option>
                                <option value="catalogue_ready" {{ request('status') == 'catalogue_ready' ? 'selected' : '' }}>Catalogue Ready</option>
                    ...
                    <table class="w-full">
                        <thead class="bg-gradient-to-r from-primary-900 to-primary-800 text-white">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Price Range</th>
                            ...
                            @forelse($items as $item)
                                <tr class="hover:bg-primary-50 transition">
                                    <td class="px-6 py-4">
                                        @if($item->primaryImage)
                                            <img src="{{ Storage::url($item->primaryImage->path) }}" 
                                                 alt="{{ $item->title }}"
                                                 class="w-16 h-16 object-cover rounded-lg shadow-md border-2 border-primary-200">
                                        @else
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-sm text-gray-600">{{ Str::limit($item->short_description, 50) }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        @php
                                            $statusColors = [
                                                ...
                                                'description' => 'bg-yellow-100 text-yellow-800',
                                                'catalogue_ready' => 'bg-purple-100 text-purple-800',
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <div class="text-gray-900 font-semibold">Est: £{{ number_format($item->estimated_price ?? 0, 2) }}</div>
                                        <div class="text-gray-600">Res: £{{ number_format($item->reserve_price ?? 0, 2) }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        ...
                                        <p class="mt-4 text-lg font-semibold text-gray-900">No items found</p>
                                        <p class="mt-2 text-sm text-gray-600">Get started by creating your first item</p>
                                        <div class="mt-6">
                                            <x-buttons.link :href="route('admin.items.create')">
                                                Create New Item
                                            </x-buttons.link>
