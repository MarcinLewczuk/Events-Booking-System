        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Catalogue Approvals</h1>
                        <p class="mt-1 text-sm text-gray-600">Review and approve catalogues awaiting approval</p>
                    </div>
                    <div class="flex items-center space-x-3">
                        <span class="px-4 py-2 bg-yellow-100 text-yellow-800 rounded-lg font-semibold">
                            {{ $catalogues->total() }} Pending
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <form method="GET" action="{{ route('admin.approvals.catalogues') }}" x-data="{ showFilters: {{ request()->hasAny(['category_id', 'created_by', 'min_items', 'sort_by']) ? 'true' : 'false' }} }">
                    <!-- Search Bar -->
                    <div class="flex gap-3 mb-4">
                        ...
                        @if(request()->hasAny(['search', 'category_id', 'created_by', 'min_items']))
                            <a href="{{ route('admin.approvals.catalogues') }}" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                                Clear
                            </a>
                        @endif
                    </div>
                    ...
                    <div x-show="showFilters" x-collapse>
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4 border-t border-gray-200">
                            ...
                            <div>
                                ...
                                <input type="number" name="min_items" value="{{ request('min_items') }}" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#370671] focus:border-transparent">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="sort_by" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#370671] focus:border-transparent">
                                    <option value="updated_at" {{ request('sort_by') == 'updated_at' ? 'selected' : '' }}>Submission Date</option>
            ...
            @if($catalogues->isEmpty())
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                    ...
                    <p class="text-gray-600">
                        ...
                        @else
                            There are no catalogues awaiting approval at this time.
                        @endif
                    </p>
                </div>
            @else
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
                            <div class="p-6">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <h3 class="text-xl font-semibold text-gray-900 mb-2">{{ $catalogue->name }}</h3>
                                        
                                        @if($catalogue->description)
                                            <p class="text-sm text-gray-600 mb-4">{{ Str::limit($catalogue->description, 150) }}</p>
                                        @endif
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                            ...
                                            <div class="flex items-center text-sm text-gray-600">
                                                ...
                                                <div>
                                                    ...
                                                    {{ $catalogue->creator?->full_name ?? 'Unknown' }}
                                                </div>
                                            </div>
                                        </div>

                                        <div class="text-xs text-gray-500">
                                            Submitted {{ $catalogue->updated_at?->diffForHumans() }}
                                        </div>
                                    </div>

                                    <div class="ml-6">
                                        <x-buttons.primary href="{{ route('admin.approvals.catalogue', $catalogue) }}">
                                            Review & Approve
                                        </x-buttons.primary>
                                    </div>
                                </div>
