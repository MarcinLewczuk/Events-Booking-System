...
<x-layouts.app>
    ...
    <div class="relative bg-primary-500 text-white py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    @if($event->category)
                        <span class="inline-block px-4 py-1 bg-secondary-600 text-white text-sm font-semibold mb-4">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8">
        @if($hasImages)
            <!-- Image Carousel -->
            <div class="relative h-96 overflow-hidden shadow-2xl" x-data="{ currentSlide: 0, totalSlides: {{ $imageCount }} }">
                <!-- Carousel Images -->
                ...
                @if($imageCount > 1)
                    ...
                    <button @click="currentSlide = currentSlide === 0 ? totalSlides - 1 : currentSlide - 1"
                            class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-3 backdrop-blur-sm transition z-10">
                    ...
                    <button @click="currentSlide = currentSlide === totalSlides - 1 ? 0 : currentSlide + 1"
                            class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-3 backdrop-blur-sm transition z-10">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                        </svg>
                    </button>
                    ...
                    <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 z-10">
                        @foreach($allImages as $index => $imagePath)
                            <button @click="currentSlide = {{ $index }}"
                                    ...
                                    class="w-2.5 h-2.5 transition"></button>
                        @endforeach
                    </div>

                    <!-- Image Counter -->
                    <div class="absolute top-4 right-4 bg-black/60 text-white px-3 py-1.5 text-sm font-medium backdrop-blur-sm z-10">
        ...
        @else
            ...
            <div class="relative h-96 overflow-hidden shadow-2xl">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white shadow-lg p-8 sticky top-24">
                    ...
                    <a href="{{ route('events.book', $event->id) }}"
                       class="block w-full bg-secondary-500 hover:bg-secondary-600 text-gray-900 py-4 transition font-bold text-lg mb-4 shadow-lg text-center">
                        Book Tickets
                    </a>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        ...
                        <div class="flex gap-2">
                            <button class="flex-1 bg-blue-600 text-white py-2 hover:bg-blue-700 transition">
                                Facebook
                            </button>
                            <button class="flex-1 bg-sky-500 text-white py-2 hover:bg-sky-600 transition">
                                Twitter
                            </button>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @foreach($relatedEvents as $relatedEvent)
                        <a href="{{ route('events.show', $relatedEvent->id) }}"
                           class="bg-white shadow-lg overflow-hidden hover:shadow-xl transition group">
                            <div class="relative h-48 bg-gradient-to-br from-primary-200 to-primary-300">
                                @if($relatedEvent->primary_image)
                                    <img src="{{ asset('storage/' . $relatedEvent->primary_image) }}"
