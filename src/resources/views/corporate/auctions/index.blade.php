    ...
    <div class="min-h-screen bg-gray-50 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                ...
                <p class="mt-2 text-gray-600">Discover upcoming auction events and their catalogues</p>
            </div>

            <!-- Recommended Auctions for Logged In Users -->
            @if($recommendedAuctions->isNotEmpty())
                <div class="mb-8 bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg p-6 border-l-4 border-primary-700">
                    <div class="flex items-center mb-4">
                        ...
                        <h2 class="text-xl font-semibold text-gray-900">Recommended Auctions For You</h2>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        @foreach($recommendedAuctions as $recAuction)
                            <a href="{{ route('auctions.show', $recAuction) }}" class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow p-4">
                                <div class="flex items-start justify-between mb-2">
                                    <h3 class="font-semibold text-gray-900">{{ $recAuction->title }}</h3>
                                    <span class="bg-purple-600 text-white text-xs font-bold px-2 py-1 rounded-full whitespace-nowrap ml-2">
                                        {{ round($recAuction->recommendation_score) }}% Match
                                    </span>
                                </div>
                                <p class="text-sm text-gray-600 mb-2">{{ $recAuction->catalogue?->name }}</p>
                                <div class="flex items-center text-sm text-gray-500">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                    </svg>
                                    {{ $recAuction->auction_date->format('M d, Y') }} at {{ $recAuction->start_time }}
                                </div>
                                <p class="text-xs text-primary-600 mt-2">{{ $recAuction->matching_items_count }} items match your interests</p>
                            </a>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-lg shadow-sm p-6 sticky top-6">
                        ...
                        <form method="GET" action="{{ route('auctions.browse') }}" id="filterForm">
                            
                            <!-- Search -->
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Search</label>
                                <input type="text" name="search" value="{{ $search }}" placeholder="Search auctions..." 
                            ...
                            <div class="mb-4">
                                ...
                                <select name="status" class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                    <option value="upcoming" {{ $status == 'upcoming' ? 'selected' : '' }}>Upcoming Only</option>
                                    <option value="scheduled" {{ $status == 'scheduled' ? 'selected' : '' }}>Scheduled/Open</option>
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Catalogue Category</label>
                            ...
                            <div class="mb-4">
                                ...
                                <!-- Prevent auto-select after clearing -->
                            ...
                            <div class="space-y-2">
                                ...
                                <a href="{{ route('auctions.browse') }}" class="block w-full text-center bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300 transition">
                                    Clear All
                                </a>
                            </div>
                        </form>
                ...
                <div class="lg:col-span-3">
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-4 mb-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div class="text-sm text-gray-600">
                            Showing <span class="font-semibold">{{ $auctions->firstItem() ?? 0 }}</span> to 
                            <span class="font-semibold">{{ $auctions->lastItem() ?? 0 }}</span> of 
                            <span class="font-semibold">{{ $auctions->total() }}</span> auctions
                        </div>

                        <!-- Sort By -->
                        <form method="GET" action="{{ route('auctions.browse') }}" class="flex items-center gap-2">
                            <!-- Preserve filters -->
                    ...
                    @if($auctions->isEmpty())
                        <div class="bg-white rounded-lg shadow-sm p-12 text-center">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 mb-2">No auctions found</h3>
                            <p class="text-gray-600 mb-4">Try adjusting your filters or check back later for upcoming auctions</p>
                            <a href="{{ route('auctions.browse') }}" class="inline-block bg-primary-700 text-white px-6 py-2 rounded-md hover:bg-primary-800 transition">
                                Clear Filters
                            </a>
                        </div>
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($auctions as $auction)
                                <a href="{{ route('auctions.show', $auction) }}" class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden group">
                                    <!-- Header -->
                                    <div class="h-32 bg-gradient-to-r from-primary-700 to-primary-600 p-4 text-white relative">
                                        @auth
                                            @if(isset($auction->recommendation_score) && $auction->recommendation_score > 0)
                                                <div class="absolute top-2 right-2 bg-purple-900 text-white text-xs font-bold px-2 py-1 rounded-full">
                                                    {{ round($auction->recommendation_score) }}% Match
                                                </div>
                                            @endif
                                        @endauth
                                        <h3 class="font-bold text-lg mb-1">{{ $auction->title }}</h3>
                                    ...
                                    <div class="p-4">
                                        ...
                                        @if($auction->catalogue)
                                            <div class="pt-3 border-t border-gray-200">
                                                <div class="flex items-center justify-between text-sm">
                                                    <span class="text-gray-600">Items in catalogue:</span>
                                                    <span class="font-semibold text-gray-900">{{ $auction->catalogue->items->count() }}</span>
                                                </div>
                                                @if($auction->catalogue->category)
                                                    <div class="flex items-center justify-between text-sm mt-1">
                                                        <span class="text-gray-600">Category:</span>
                                                        <span class="text-gray-900">{{ $auction->catalogue->category->name }}</span>
                                                    </div>
                                                @endif
                                                @auth
                                                    @if(isset($auction->matching_items_count) && $auction->matching_items_count > 0)
                                                        <div class="mt-2 bg-purple-50 px-3 py-2 rounded text-xs text-primary-700 font-medium">
                                                            {{ $auction->matching_items_count }} {{ Str::plural('item', $auction->matching_items_count) }} match your interests
                                                        </div>
                                                    @endif
                                                @endauth
                                            </div>
                                        ...
                                        <div class="mt-3">
                                            @php
                                                ...
                                                $statusColor = $statusColors[$auction->status] ?? 'bg-gray-100 text-gray-800';
                                            @endphp
                                    ...
                                    <div class="px-4 py-3 bg-gray-50 border-t border-gray-200">
                                        <span class="text-sm font-medium text-primary-700 group-hover:text-primary-900 flex items-center">
                                            View Details
