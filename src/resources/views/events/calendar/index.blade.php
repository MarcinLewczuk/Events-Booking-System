        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 bg-white rounded-lg shadow-sm p-6 border-l-4 border-primary-600">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">📅 My Events Calendar</h1>
                        <p class="text-gray-600 mt-1">View all your confirmed bookings</p>
                    </div>
                    <div class="flex gap-4">
                        <a href="{{ route('calendar.browse-all') }}" class="text-primary-600 hover:text-primary-700 font-semibold">
                            Browse All Events →
                        </a>
                        <a href="{{ route('customer.bookings') }}" class="text-primary-600 hover:text-primary-700 font-semibold">
                            My Bookings →
                        </a>
                        <a href="{{ route('dashboard') }}" class="text-gray-600 hover:text-gray-700 font-semibold">
                            ← Back
                        </a>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr style="background-color: #247a7c;">
                            ...
                            <tr>
                                @for($i = 0; $i < 7; $i++)
                                    @php
                                        ...
                                        $dayEvents = $bookings->filter(function($booking) use ($currentIterDate) {
                                            return $booking->event->start_datetime->format('Y-m-d') === $currentIterDate->format('Y-m-d');
                                        })->values();
                                    @endphp
                                    <td class="border border-gray-200 p-2 align-top {{ $isCurrentMonth ? 'bg-white' : 'bg-gray-50' }}" style="height: 110px; {{ $isToday ? 'background-color: #e6f4f5; outline: 2px solid #247a7c; outline-offset: -2px;' : '' }}">
                                        <div class="mb-1">
                                            @if($isToday)
                                                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold text-white" style="background-color: #247a7c;">{{ $currentIterDate->day }}</span>
                                            @else
                                                <span class="text-sm font-bold {{ $isCurrentMonth ? 'text-gray-900' : 'text-gray-400' }}">{{ $currentIterDate->day }}</span>
                                            @endif
                                        </div>
                                        @foreach($dayEvents as $booking)
                                            <a href="{{ route('events.show', $booking->event) }}" 
                                               ...
                                               style="background-color: #247a7c;"
                                               title="{{ $booking->event->title }} - {{ $booking->event->start_datetime->format('g:i A') }}">
                                                {{ $booking->event->title }}
                                            </a>
                                        @endforeach
                                    </td>
                ...
                <div class="p-4 border-t border-gray-200 bg-gray-50">
                    <div class="flex items-center gap-6 flex-wrap text-sm">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-gray-700">Your Booked Events</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="inline-flex items-center justify-center w-5 h-5 rounded-full text-white text-xs font-bold" style="background-color: #247a7c;">12</span>
            ...
            @if($bookings->count() > 0)
                <div class="mt-8 bg-white rounded-lg shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-gray-200">
                        <h3 class="text-xl font-bold text-gray-900">All Your Bookings</h3>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @foreach($bookings->sortByDesc(function($b) { return $b->event->start_datetime; }) as $booking)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-6 py-4 text-gray-600">
                                            {{ $booking->event->location->name ?? 'TBA' }}
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 text-center space-x-2">
                                            <a href="{{ route('events.show', $booking->event) }}" 
                                               class="text-primary-600 hover:text-primary-700 font-semibold text-xs">
                                                View
                                            </a>
                                            <a href="{{ route('calendar.export-ics', $booking) }}" 
                                               class="text-gray-600 hover:text-gray-700 font-semibold text-xs">
                                                Download
                                            </a>
                                        </td>
                                    </tr>
